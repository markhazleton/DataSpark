# DataSpark Benchmarks

BenchmarkDotNet benchmarks for the core DataSpark services (discovery, export, schema generation).
...
```bash
dotnet run -c Release --project DataSpark.Benchmarks
```

Add a filter (optional):

```bash
dotnet run -c Release --project DataSpark.Benchmarks -- --filter *Export*
```
