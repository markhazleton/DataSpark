...
public class DatabaseAnalysisService : IDatabaseAnalysisService
{
    ...
    private readonly ILogger<DatabaseAnalysisService> _logger;
    private readonly string _tempDirectory;
    private readonly HashSet<string> _tempFiles = [];

    public DatabaseAnalysisService(
        ISchemaService schemaService,
        IExportService exportService,
        ICodeGenerationService codeGenerationService,
        ILogger<DatabaseAnalysisService> logger)
    {
        ...
        _tempDirectory = Path.Combine(Path.GetTempPath(), "DataSpark.Core");
        Directory.CreateDirectory(_tempDirectory);
    }

    public async Task<FileValidationResult> ValidateDatabaseFileAsync(string filePath, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return FileValidationResult.Failed("Invalid SQLite file header.");
            }
            ...
            return FileValidationResult.Successful(tableCount);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error validating database file: {FilePath}", filePath);
            return FileValidationResult.Failed($"Invalid SQLite database file: {ex.Message}");
        }
    }

    public async Task<DatabaseAnalysisResult> AnalyzeDatabaseAsync(string filePath, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var tableModels = new List<TableInfo>();
            foreach (var table in tablesInfo)
            {
                var columns = table.Columns.Select(c => new ColumnInfo
                {
                    Name = c.Name,
                    DataType = c.DataType,
    ...
    public async Task<List<ExportResult>> ExportTablesToCsvAsync(string filePath, List<string> tableNames, CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<List<GeneratedCodeResult>> GenerateCodeAsync(string filePath, List<string> tableNames, string namespaceName, CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<TableAnalysisResult> AnalyzeTableAsync(string filePath, string tableName, CancellationToken cancellationToken = default)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
    ...
    public async Task<TableDataResult> GetTableDataAsync(string filePath, string tableName, DataTablesRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var hasSearch = !string.IsNullOrEmpty(whereClause);
            var searchPattern = hasSearch ? $"%{request.SearchValue}%" : null;
            ...
            if (hasSearch)
            {
                ...
                using var filteredCountCommand = new SqliteCommand(filteredCountSql, connection);
                filteredCountCommand.Parameters.AddWithValue("@searchPattern", searchPattern!);
                var filteredCountResult = await filteredCountCommand.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);
                filteredRecords = filteredCountResult == null || filteredCountResult == DBNull.Value
                    ? 0
                    : Convert.ToInt64(filteredCountResult);
            }
    ...
    // Implementation of helper methods continues below...
    // [Additional helper methods for table statistics, column analysis, etc.]
    ...
    private async Task<ColumnAnalysis> AddNumericAnalysisAsync(SqliteConnection connection, string tableName, string columnName, ColumnAnalysis analysis, CancellationToken cancellationToken)
    {
        var numericStatsSql = $@"
            SELECT 
        ...
        analysis.MinValue = minValue?.ToString();
        analysis.MaxValue = maxValue?.ToString();
        analysis.MeanValue = meanValue;
        analysis.MedianValue = medianValue;

        return analysis;
    }

    private async Task<ColumnAnalysis> AddTextAnalysisAsync(SqliteConnection connection, string tableName, string columnName, ColumnAnalysis analysis, CancellationToken cancellationToken)
    {
        var textStatsSql = $@"
            SELECT 
        ...
        analysis.MinLength = minLength;
        analysis.MaxLength = maxLength;
        analysis.AverageLength = averageLength;

        return analysis;
    }

    private async Task<ColumnAnalysis> AddDateTimeAnalysisAsync(SqliteConnection connection, string tableName, string columnName, ColumnAnalysis analysis, CancellationToken cancellationToken)
    {
        var dateStatsSql = $@"
            SELECT 
