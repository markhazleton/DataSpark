...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Upload(FileUploadViewModel model)
    {
        ...
        else if (model.DatabaseFile == null)
        {
            ...
            ModelState.AddModelError(nameof(model.DatabaseFile), "Please select a data file or choose an existing file.");
        }
        ...
        try
        {
            ...
            DataSourceType fileType = DataSourceType.Database;
            ...
            else if (model.DatabaseFile != null)
            {
                ...
                if (model.SaveForFutureUse && !string.IsNullOrEmpty(uploadedFilePath))
                {
                    try
                    {
            ...
            {
                model.ErrorMessage = "Please select a data file or choose an existing file.";
                model.AvailableFiles = await _persistedFileService.GetAvailableFilesAsync();
                return View("Index", model);
            }

            // Store the file information in TempData for the next request
            TempData["DataFilePath"] = filePath;
            TempData["DataFileName"] = fileName;
            TempData["DataFileType"] = fileType.ToString();
            
            // Store additional info for display
            if (additionalInfo != null)
            {
                TempData["AdditionalInfo"] = System.Text.Json.JsonSerializer.Serialize(additionalInfo);
            }
    ...
    public async Task<IActionResult> UploadApi(IFormFile? file, bool persist = false, string? description = null, CancellationToken cancellationToken = default)
    {
        ...
        var (success, errorMessage, filePath, fileType, additionalInfo) = await _unifiedDataService.SaveUploadedFileAsync(file, cancellationToken);
        ...
        TempData["DataFilePath"] = filePath;
        TempData["DataFileName"] = file.FileName;
        TempData["DataFileType"] = fileType.ToString();

        // Create response with file type specific information
        var response = new
        {
            id = persistedId,
            name = file.FileName,
    ...
    public async Task<IActionResult> Analyze()
    {
        ...
        _logger.LogInformation("Analyze action called with FilePath: {FilePath}, FileName: {FileName}, FileType: {FileType}", 
            filePath, fileName, fileTypeString);
        ...
        {
            fileType = DataSourceType.Database; // Default to database for backward compatibility
        }
        ...
        if (!System.IO.File.Exists(filePath))
        {
            ...
            TempData["ErrorMessage"] = "The selected data file is no longer available.";
            return RedirectToAction("Index");
        }

        try
        {
            _logger.LogInformation("Starting unified analysis for file: {FilePath} ({FileType})", filePath, fileType);
            ...
            {
                // Use existing database analysis for backward compatibility
            ...
            {
                TempData["ErrorMessage"] = $"Unsupported file type: {fileType}";
                return RedirectToAction("Index");
            }
        }
        catch (OperationCanceledException)
        {
            _logger.LogError("Data analysis timed out for file: {FilePath}", filePath);
            TempData["ErrorMessage"] = "Data analysis timed out. The file might be too large or corrupted.";
            return RedirectToAction("Index");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error analyzing data file: {FilePath}", filePath);
            TempData["ErrorMessage"] = "An error occurred while analyzing the data file.";
            return RedirectToAction("Index");
        }
    }
    ...
    public async Task<IActionResult> GetUnifiedData([FromForm] DataTablesRequest request)
    {
        ...
        try
        {
            _logger.LogInformation("Getting unified data for file: {FilePath} ({FileType}), Start: {Start}, Length: {Length}",
                filePath, fileType, request.Start, request.Length);
            ...
            if (fileType == DataSourceType.Database)
            {
                // For database files, we'd need to know which table - this is a simplified approach
                tableName = HttpContext.Session.GetString("CurrentTableName");
            }
        ...
        {
            _logger.LogError("GetUnifiedData timed out for file: {FilePath}", filePath);
            return Json(new TableDataResult { Error = "Operation timed out" });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting unified data for file: {FilePath}", filePath);
