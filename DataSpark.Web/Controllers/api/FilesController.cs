    ...
    public IActionResult BivariateSvg([FromForm] string fileName, [FromForm] string column1, [FromForm] string column2)
    {
        ...
        try
        {
            ...
            if (col1Numeric && col2Numeric)
            {
                ...
                if (pairs.Count < 2)
                    return BadRequest("Insufficient numeric points for scatter plot.");
            ...
            return Content(plot.GetSvgXml(900, 500), "image/svg+xml");
        }
        catch (Exception ex)
        {
            return StatusCode(500, $"Failed to generate bivariate SVG: {ex.Message}");
        }
    }
